  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.presetsList}>
          {presets.map((preset, index) => (
            <View key={index} style={styles.presetCard}>
              ...
              <Text style={styles.label}>{getPresetLabel()} Number *</Text>
              <TextInput
                style={styles.input}
                value={String(
                  deviceType === "nord_stage_4"
              ...
              <Text style={styles.hint}>
                💡 Tip: Use an existing {getPresetLabel().toLowerCase()} number
                from your keyboard
              </Text>
            </View>
          ))}
      ...
      <TouchableOpacity style={styles.addButton} onPress={addPreset}>
        <Text style={styles.addButtonText}>➕ Add {getPresetLabel()}</Text>
      </TouchableOpacity>

      <View style={styles.infoBox}>
        <Text style={styles.infoTitle}>📝 Phase 1 Note:</Text>
        <Text style={styles.infoText}>
          In Phase 1, we recall existing {getPresetLabel().toLowerCase()}s on
          your keyboard.
        </Text>
        <Text style={styles.infoText}>
          Phase 2 will add: browse keyboard library, create new{" "}
          {getPresetLabel().toLowerCase()}s, and more!
        </Text>
      </View>
      ...
      {presets.length > 0 && (
        ...
        >
          ...
          <Text style={styles.mapSectionsHint}>
            Choose which preset to use for each section (Intro, Verse,
            Chorus...)
          </Text>
        </TouchableOpacity>
      )}
