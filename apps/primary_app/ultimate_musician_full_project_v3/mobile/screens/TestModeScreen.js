...
export default function TestModeScreen({ route, navigation }) {
    ...
    try {
      ...
      if (result.status === "success" || result.status === "partial_success") {
        ...
        if (result.errors && result.errors.length > 0) {
          ...
          Alert.alert(
            "Success",
            `${result.triggered_devices.length} device(s) triggered!`,
          );
        }
      } else {
  ...
  const renderDeviceSetup = (role, deviceType, deviceSetup) => {
    ...
    const isConnected =
      devices?.detected_devices?.[deviceType.replace("_4", "")] != null;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.header}>
        ...
        {song.artist && <Text style={styles.subtitle}>{song.artist}</Text>}
      </View>
        ...
        <>
          {Object.entries(deviceSetups).map(([role, roleDevices]) =>
            Object.entries(roleDevices).map(([deviceType, deviceSetup]) =>
              deviceSetup
                ? renderDeviceSetup(role, deviceType, deviceSetup)
                : null,
            ),
          )}
          ...
          <TouchableOpacity
            style={[
              ...
              testing && styles.triggerButtonDisabled,
            ]}
            onPress={testPreset}
            disabled={testing}
          >
