...
export default function BlockoutCalendarScreen({ navigation }) {
  const { isGuest, userId, userName } = useAuth();
  ...
  const myBlockedSet = useMemo(
    () => new Set(myBlockouts.map((b) => b.date)),
    [myBlockouts],
  );
  ...
  async function handleRemove(blockout) {
    Alert.alert("Remove blockout?", formatDisplayDate(blockout.date), [
      ...
      >
        ...
        <View style={styles.infoBanner}>
          <Text style={styles.infoText}>
            Tap a date to mark yourself as unavailable. Your admin will see
            blocked dates when scheduling services.
          </Text>
        </View>
          ...
          {upcomingBlockouts.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyText}>
                No upcoming blockouts. Tap a date above to mark yourself as
                unavailable.
              </Text>
            </View>
          ) : (
