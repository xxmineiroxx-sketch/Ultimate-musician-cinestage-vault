...
export default function DronePadScreen({ navigation }) {
  ...
  const activeSemitones =
    typeof session.pitchShiftSemitones === "number"
      ? session.pitchShiftSemitones
      : 0;

  const activeNote = useMemo(() => {
    const match = NOTES.find(
      (n) => n.semitones === ((activeSemitones % 12) + 12) % 12,
    );
      ...
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.rowInline}>
            <TouchableOpacity
              style={styles.pill}
              onPress={() => updatePadVolume(padVolume - 0.1)}
            >
        ...
        <Text style={styles.subtle}>
          Current key: {activeNote} ({activeSemitones} semitones)
        </Text>
        <Text style={styles.noteHint}>
          Pad key selection is sent to Bridge HQ for real-time pitch shift.
          Local pad playback uses the generated pad track.
        </Text>
      </View>
          ...
          {NOTES.map((n) => (
            ...
            >
              <Text
                style={[
                  styles.keyText,
                  activeNote === n.label && styles.keyTextActive,
                ]}
              >
                {n.label}
              </Text>
