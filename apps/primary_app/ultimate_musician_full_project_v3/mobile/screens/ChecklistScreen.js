...
import { getServicePlan, getSongs, getServices } from "../data/storage";
    ...
    (async () => {
      ...
      for (const tags of songTagsList) {
        (tags || "")
          ...
          .forEach((t) => {
            const tt = t.trim();
            if (THEME_MAP[tt] && !detectedNames.includes(tt))
              detectedNames.push(tt);
          });
      }
    ...
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.container}>
          ...
          <>
            ...
            {suggested.map((item) => (
              <Pressable
                ...
                style={[
                  styles.item,
                  item.done ? styles.itemDoneSuggested : styles.itemSuggested,
                ]}
              >
        ...
        <Text
          style={[
            styles.sectionLabel,
            { marginTop: suggested.length > 0 ? 18 : 4 },
          ]}
        >
          Standard Checklist
        </Text>
        {standard.map((item) => (
          <Pressable
            ...
            style={[
              styles.item,
              item.done ? styles.itemDone : styles.itemDefault,
            ]}
          >
