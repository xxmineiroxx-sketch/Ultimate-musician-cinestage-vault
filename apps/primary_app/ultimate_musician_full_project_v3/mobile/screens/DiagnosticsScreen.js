...
export default function DiagnosticsScreen({ navigation }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Sent (offline mode)",
        "Diagnostics collected. Server was not reachable, but the report is ready to share manually.",
      );
      setSent(true);
    }
    setSending(false);
    ...
    return (
      ...
      >
        ...
        <Text style={s.sentText}>
          The admin will receive your report shortly.
        </Text>
  ...
  return (
    ...
    >
      <ScrollView
        contentContainerStyle={[s.body, { paddingTop: insets.top + 16 }]}
        keyboardShouldPersistTaps="handled"
      >
        <Text style={s.pageTitle}>Support & Diagnostics</Text>
        <Text style={s.subtitle}>
          Send a diagnostics report to the admin. Includes app version, storage
          info, and your notes.
        </Text>
        ...
        {loading ? (
          ...
          <View style={s.diagCard}>
            ...
            <View style={s.diagRow}>
              <Text style={s.diagKey}>App Version</Text>
              <Text style={s.diagVal}>{diag.appVersion}</Text>
            </View>
            <View style={s.diagRow}>
              <Text style={s.diagKey}>Platform</Text>
              <Text style={s.diagVal}>
                {diag.platform} {diag.osVersion}
              </Text>
            </View>
            <View style={s.diagRow}>
              <Text style={s.diagKey}>Storage Keys</Text>
              <Text style={s.diagVal}>{diag.storageKeys.length} keys</Text>
            </View>
            <View style={s.diagRow}>
              <Text style={s.diagKey}>Total Storage</Text>
              <Text style={s.diagVal}>
                {(diag.totalStorage / 1024).toFixed(1)} KB
              </Text>
            </View>
            <View style={s.diagRow}>
              <Text style={s.diagKey}>Timestamp</Text>
              <Text style={s.diagVal}>
