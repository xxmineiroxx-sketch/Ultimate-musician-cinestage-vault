...
import { SYNC_URL as GLOBAL_SYNC_URL } from "./config";
...
export default function SystemMapScreen({ navigation }) {
  ...
  const pingAll = async () => {
    setPinging(true);
    const SYNC_URL = GLOBAL_SYNC_URL;
    const CINE_URL = `http://${settings.cinestageHost || "10.0.0.34:8000"}`;

    const checks = await Promise.allSettled([
      fetchWithTimeout(`${SYNC_URL}/sync/debug`, 3000),
      fetchWithTimeout(`${CINE_URL}/health`, 3000),
    ]);
    ...
    setDevices((prev) =>
      prev.map((d, i) => {
        if (d.id === "d1")
          return {
            ...d,
            status: checks[0].status === "fulfilled" ? "active" : "offline",
          };
        if (d.id === "d2")
          return {
            ...d,
            status: checks[1].status === "fulfilled" ? "active" : "offline",
          };
        return d;
      }),
    );
      ...
      <ScrollView contentContainerStyle={s.body}>
        ...
        {devices.map((device) => (
          <View key={device.id} style={s.card}>
            ...
            <View
              style={[
                s.statusDot,
                { backgroundColor: STATUS_COLOR[device.status] || "#6B7280" },
              ]}
            />
          </View>
        ))}
        ...
        <View style={s.tip}>
          <Text style={s.tipText}>
            💡 Tap "Ping All" to check live status of Sync Server and CineStage
            AI. All devices must be on the same Wi-Fi network.
          </Text>
        </View>
      </ScrollView>
    </View>
