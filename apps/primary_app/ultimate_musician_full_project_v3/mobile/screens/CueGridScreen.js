...
import {
  autoAssignLyricsCues,
  autoAssignLightingCues,
} from "../services/cueMapper";
import {
  loadSession,
  saveSession,
  defaultSession,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.p}>
          Edit lyricsCue + lightingCue + lightingColor in one place. Great for
          producers.
        </Text>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
          <TouchableOpacity
            style={styles.btn}
            onPress={() => persistMarkers(autoAssignLyricsCues(markers, 1, 1))}
          >
          ...
          <TouchableOpacity
            style={styles.btnAlt}
            onPress={() =>
              persistMarkers(autoAssignLightingCues(markers, 1, 1))
            }
          >
        ...
        {markers.map((m) => (
          <View key={m.id} style={styles.gridRow}>
            ...
            <TextInput
              ...
              onChangeText={(t) =>
                updateMarker(m.id, { lyricsCue: t === "" ? null : Number(t) })
              }
            ...
            <TextInput
              ...
              onChangeText={(t) =>
                updateMarker(m.id, { lightingCue: t === "" ? null : Number(t) })
              }
              keyboardType="numeric"
              placeholder="—"
              placeholderTextColor={colors.subtle}
