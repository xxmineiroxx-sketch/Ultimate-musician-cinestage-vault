...
import {
  transposeChordChart,
  stripChordsForVocals,
} from "../data/chordTranspose";
import {
  INSTRUMENT_SHEETS,
...
import {
  ...
  updateSongItem,
  addVocalAssignment,
  removeVocalAssignment,
  distributeChordChart,
  updateVoicePartAudio,
  removeVoicePartAudio,
...
export default function SongPlanDetailScreen({ route, navigation }) {
  ...
  const [showTransposedPreview, setShowTransposedPreview] = useState(false);
  ...
  async function uploadAudioForPart(voicePart) {
    ...
    try {
      ...
      Alert.alert(
        "Upload failed",
        e.message || "Could not import the audio file.",
      );
    } finally {
  ...
  async function handleRemoveAudio(voicePart) {
    ...
    Alert.alert(
      `Remove ${voicePart} audio?`,
      "This will delete the uploaded reference track.",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (audioInfo?.localUri) {
              await FileSystem.deleteAsync(audioInfo.localUri, {
                idempotent: true,
              });
            }
            const next = await removeVoicePartAudio(
              serviceId,
              itemId,
              voicePart,
            );
  ...
  async function handleAssignVocalist(person) {
    if (!assignVoicePart) {
      Alert.alert(
        "Select voice part",
        "Choose a voice part (Soprano, Alto, Tenor…) before assigning.",
      );
      return;
    }
  ...
  async function handleRemoveVocal(assignmentId) {
    Alert.alert("Remove vocalist?", "", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          const next = await removeVocalAssignment(
            serviceId,
            itemId,
            assignmentId,
          );
    ...
    return (
      <View style={styles.centered}>
        <Text style={styles.notFoundText}>
          Song not found in this service plan.
        </Text>
        <TouchableOpacity
  ...
  const keyChanged =
    isDistributed && item.distributedInKey !== (toKey || fromKey);
    ...
    <View style={styles.root}>
      ...
      <View style={styles.songHeader}>
        ...
        {keyChanged && (
          <View style={[styles.sharedBadge, styles.sharedBadgeWarn]}>
            <Text style={[styles.sharedBadgeText, { color: "#FCD34D" }]}>
              ⚠ Key changed
            </Text>
          </View>
        )}
      ...
      {tab === "chordChart" && (
        <View style={{ flex: 1 }}>
          <View style={styles.chartBar}>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setVocalistPreview(true)}
              >
                <Text style={styles.previewBtnText}>👁 Vocals</Text>
              </TouchableOpacity>
              {toKey && toKey !== fromKey && (
                <TouchableOpacity
                  style={[styles.previewBtn, showTransposedPreview && styles.previewBtnActive]}
                  onPress={() => setShowTransposedPreview((v) => !v)}
                >
                  <Text style={[styles.previewBtnText, showTransposedPreview && { color: '#34D399' }]}>
                    {showTransposedPreview ? '✓ ' : '🔑 '}{toKey}
                  </Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity
                style={[
                  ...
                  (!chordChart.trim() || savingChart) && { opacity: 0.4 },
                ]}
          ...
          {/* Key info banner — shows whenever playing key differs from original */}
          {toKey && toKey !== fromKey && (
            <View style={styles.keyInfoBanner}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Text style={styles.keyInfoBannerOriginal}>{fromKey || '—'}</Text>
                <Text style={{ color: '#4B5563', fontSize: 12 }}>→</Text>
                <Text style={styles.keyInfoBannerPlaying}>{toKey}</Text>
                <Text style={styles.keyInfoBannerLabel}>
                  {isDistributed && !keyChanged ? '  ✓ Chart distributed in ' + item.distributedInKey : '  (playing key)'}
                </Text>
              </View>
              {!isDistributed && (
                <Text style={styles.keyInfoBannerSub}>
                  Chart below is in <Text style={{ color: '#F9FAFB' }}>{fromKey}</Text>. Tap 🎸 Share to auto-transpose to <Text style={{ color: '#34D399' }}>{toKey}</Text> for each instrument.
                </Text>
              )}
            </View>
          )}
          ...
          {keyChanged && (
            <View style={styles.keyChangedBanner}>
              <Text style={styles.keyChangedBannerText}>
                ⚠️ Key changed to {toKey}. Tap 🎸 Share to re-distribute
                transposed charts.
              </Text>
            </View>
          )}
          ...
          {/* Transposed preview — live auto-transpose of the chart to playing key */}
          {showTransposedPreview && toKey && toKey !== fromKey && chordChart.trim() && (
            <ScrollView
              style={styles.transposedPreview}
              contentContainerStyle={{ padding: 14 }}
              showsVerticalScrollIndicator={false}
            >
              <Text style={styles.transposedPreviewLabel}>🔑 Preview in {toKey}:</Text>
              <Text style={styles.transposedPreviewText}>
                {transposeChordChart(chordChart, fromKey, toKey)}
              </Text>
            </ScrollView>
          )}
        </View>
      ...
      {tab === "instruments" && (
        ...
        >
          <Text style={styles.hint}>
            Tap an instrument to expand. "chord" instruments are pre-filled when
            you tap 🎸 Share.
          </Text>
              ...
              <View key={inst} style={styles.accordionCard}>
                ...
                >
                  ...
                  >
                    ...
                    {isChordInst && (
                      <Text style={styles.accordionTag}>chord</Text>
                    )}
                  </View>
      ...
      {tab === "vocals" && (
        ...
        >
          ...
          <Text style={[styles.hint, { marginTop: 16 }]}>
            Upload a reference audio for each voice part so musicians can hear
            their specific line.
          </Text>

          {/* ── Voice Part Sections ────────────────────────────────────────── */}
          {VOICE_PARTS.map((part) => {
            const partAssignments = vocalAssignments.filter(
              (v) => v.voicePart === part,
            );
            ...
            return (
              <View key={part} style={styles.voicePartSection}>
                ...
                <View style={styles.voicePartHeader}>
                  <Text style={styles.voicePartLabel}>
                    {part.toUpperCase()}
                  </Text>
                ...
                {audioInfo && (
                  <TouchableOpacity
                    style={[
                      styles.audioPlayer,
                      isPlaying && styles.audioPlayerActive,
                    ]}
                ...
                {partAssignments.length === 0 && !audioInfo && (
                  <Text style={styles.voicePartEmpty}>
                    No vocalist or audio yet — tap + Audio to upload a reference
                    track.
                  </Text>
                )}
              </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <Text style={styles.previewNote}>
              Chord lines are removed automatically. This is what vocalists
              receive.
            </Text>
            <ScrollView>
            ...
            >
              {VOICE_PARTS.map((vp) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.vpChip,
                    assignVoicePart === vp && styles.vpChipActive,
                  ]}
                  onPress={() =>
                    setAssignVoicePart(assignVoicePart === vp ? "" : vp)
                  }
                >
                  <Text
                    style={[
                      styles.vpChipText,
                      assignVoicePart === vp && styles.vpChipTextActive,
                    ]}
                  >
                    {vp}
                  </Text>
            ...
            <View style={{ flexDirection: "row", marginBottom: 12 }}>
              {[
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.typeChip,
                    assignType === t.key && styles.typeChipActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.typeChipText,
                      assignType === t.key && styles.typeChipTextActive,
                    ]}
                  >
                    {t.label}
                  </Text>
                ...
                filteredPeople.map((p) => (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {(p.roles || []).length > 0 && (
                        <Text style={styles.personRoles}>
                          {p.roles.map((role) => formatRoleLabel(role)).join(" · ")}
                        </Text>
                      )}
                    </View>
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  keyInfoBannerOriginal: { fontSize: 16, fontWeight: '800', color: '#9CA3AF' },
  keyInfoBannerPlaying:  { fontSize: 16, fontWeight: '800', color: '#34D399' },
  keyInfoBannerLabel:    { fontSize: 11, color: '#4B5563' },
  keyInfoBannerSub:      { fontSize: 11, color: '#4B5563', marginTop: 4, lineHeight: 16 },
  previewBtnActive: { backgroundColor: '#14532D' },
  transposedPreview: {
    height: 220,
