...
export default function SectionMappingScreen({ route, navigation }) {
  ...
  const handleSave = async () => {
    try {
      ...
      SONG_SECTIONS.forEach((section) => {
        if (!mappings[section]) {
          updatedSong = removeSectionMapping(
            updatedSong,
            section,
            role,
            deviceType,
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.infoBox}>
        ...
        <Text style={styles.infoText}>• Map sections to specific presets</Text>
          ...
          {mappings[section] !== undefined && (
            <View style={styles.mappedBadge}>
              <Text style={styles.mappedBadgeText}>
                ✓ Mapped to{" "}
                {getPresetLabel(presets[mappings[section]], mappings[section])}
              </Text>
            </View>
          )}
        </View>
