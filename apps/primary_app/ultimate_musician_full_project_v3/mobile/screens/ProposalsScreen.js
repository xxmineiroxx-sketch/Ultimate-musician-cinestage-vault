...
const RIG_COLORS = {
  Nord: '#EF4444', MODX: '#F97316', VS: '#EAB308',
  Kontakt: '#10B981', Ableton: '#3B82F6',
};
...
async function fetchJson(url, opts = {}) {
  ...
  try {
    const res = await fetch(url, {
      ...opts,
      headers: { ...syncHeaders(), ...(opts.headers || {}) },
      signal: ctrl.signal,
    });
    clearTimeout(tid);
...
async function syncApprovedSongToLibrary(songId, songTitle) {
  try {
    const libData = await fetchJson(
      `${SYNC_URL}/sync/song-library?songId=${encodeURIComponent(songId)}`,
    );
    if (!libData || libData.length === 0) return;
    const serverSong = libData[0];

    const localSongs = await getSongs();
    const existing = localSongs.find(
      (s) =>
        s.id === serverSong.id ||
        (s.title?.toLowerCase().trim() ===
          (serverSong.title || songTitle || "").toLowerCase().trim() &&
          s.artist?.toLowerCase().trim() ===
            (serverSong.artist || "").toLowerCase().trim()),
    );

    if (existing) {
      const mergedRigs = serverSong.keyboardRigs?.length
        ? [...new Set([...(existing.keyboardRigs || []), ...serverSong.keyboardRigs])]
        : (existing.keyboardRigs || undefined);
      ...
      await addOrUpdateSong({
        ...
        ...(mergedRigs ? { keyboardRigs: mergedRigs } : {}),
      });
      console.log(
        `[ProposalsScreen] synced "${serverSong.title}" to local library`,
      );
    }
  } catch (e) {
        ...
        <ScrollView style={s.detailBody} keyboardShouldPersistTaps="handled">
          ...
          {selected.instrument && selected.instrument !== "Vocals" && (
            <View style={s.instrNoteBanner}>
              <Text style={s.instrNoteText}>
                ...
                -specific part. Approving will only update the{" "}
                {selected.instrument} notes for this song — other instruments
                are unaffected.
              </Text>
            </View>
          )}
          ...
          <View style={s.contentBox}>
            <Text
              style={[
                s.contentText,
                selected.type === "chord_chart" && s.contentMono,
              ]}
            >
          ...
          {/* Keyboard Rigs */}
          {selected.instrument === "Keys" && selected.keyboardRigs?.length > 0 && (
            <View style={s.rigsRow}>
              <Text style={s.rigsLabel}>🎹 KEYBOARD RIGS</Text>
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 6 }}>
                {selected.keyboardRigs.map((rig) => {
                  const color = RIG_COLORS[rig] || "#8B5CF6";
                  return (
                    <View key={rig} style={[s.rigChip, { borderColor: color, backgroundColor: color + "20" }]}>
                      <View style={[s.rigDot, { backgroundColor: color }]} />
                      <Text style={[s.rigChipText, { color }]}>{rig}</Text>
                    </View>
                  );
                })}
              </View>
              ...
              >
                {acting ? (
                  ...
                  <Text style={s.approveBtnText}>
                    ✓ Approve — Publish & Sync to Library
                  </Text>
                )}
              </TouchableOpacity>
    ...
    <View style={s.container}>
      ...
      <View style={s.filters}>
        {["pending", "approved", "rejected", "all"].map((f) => (
          ...
          >
            <Text
              style={[s.filterBtnText, filter === f && s.filterBtnTextActive]}
            >
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const instrIcon = item.instrument
            ? INSTR_ICON[item.instrument] || "🎼"
            : "";
...
const s = StyleSheet.create({
  ...
  rigsRow:    { marginBottom: 16 },
  rigsLabel:  { fontSize: 10, fontWeight: '700', color: '#6B7280', letterSpacing: 1 },
  rigChip:    { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 10, borderWidth: 1 },
  rigDot:     { width: 8, height: 8, borderRadius: 4 },
  rigChipText:{ fontSize: 12, fontWeight: '700' },
