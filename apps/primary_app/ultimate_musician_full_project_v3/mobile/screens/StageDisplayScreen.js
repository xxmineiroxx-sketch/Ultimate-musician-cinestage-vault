...
export default function StageDisplayScreen({ navigation }) {
  ...
  const sectionColor = state.section?.colorKey
    ? colorHex(state.section.colorKey)
    : "#4F46E5";
    ...
    <View style={styles.container}>
      <View style={styles.top}>
        ...
        <Text style={styles.meta}>
          BPM {state.bpm} • {state.status?.toUpperCase?.() || "IDLE"} •{" "}
          {state.loop ? "LOOP" : "—"}
        </Text>
      </View>
      ...
      <View style={[styles.bigCard, { borderColor: sectionColor }]}>
        ...
        <Text style={styles.bigSection}>
          {state.section ? `${state.section.type}: ${state.section.name}` : "—"}
        </Text>
      </View>
      ...
      <View style={styles.nextCard}>
        ...
        <Text style={styles.nextText}>
          {state.next ? `${state.next.type}: ${state.next.name}` : "—"}
        </Text>
      </View>
      ...
      <View style={styles.noteCard}>
        ...
        <Text style={styles.noteText}>
          {state.section?.notes || state.sectionNotes || "—"}
        </Text>
      </View>
      ...
      <View style={styles.countCard}>
        ...
        <Text style={styles.countText}>
          {state.barsToNext != null
            ? `${state.barsToNext.toFixed(1)} bars`
            : "—"}
        </Text>
      </View>
      ...
      <Text style={styles.hint}>
        Stage Display follows HOST in real-time. Connect in Device Role & Sync.
      </Text>
    </View>
  );
}
