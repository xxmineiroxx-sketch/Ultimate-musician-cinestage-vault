...
export default function ExternalSyncScreen({ navigation }) {
  ...
  const [bridgeStatus, setBridgeStatus] = useState(
    getBridgeUrl() ? "connected" : "disconnected",
  );
  ...
  function handleTestCue() {
    ...
    addLog(
      ok
        ? "→ Test cue sent: Chorus (index 1)"
        : "✕ Bridge not connected — test cue not sent",
    );
  }
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.container}>
      ...
      <Text style={styles.sub}>
        Connect to the Ultimate Bridge to send live cues to ProPresenter,
        lighting, and keep multiple devices in sync. Run the bridge scripts on a
      ...
      <View style={styles.card}>
        <View style={styles.cardRow}>
          ...
          <View
            style={[
              styles.dot,
              { backgroundColor: STATUS_COLOR[bridgeStatus] || "#6B7280" },
            ]}
          />
          <Text
            style={[
              styles.statusText,
              { color: STATUS_COLOR[bridgeStatus] || "#6B7280" },
            ]}
          >
            {bridgeStatus}
          </Text>
        ...
        <Text style={styles.cardNote}>
          Port 7070 · Sends section cues, MIDI clock, and OSC to lyric /
          lighting software
        </Text>
        ...
        <View style={styles.btnRow}>
          {bridgeStatus === "connected" ? (
            <TouchableOpacity
              ...
              onPress={handleDisconnectBridge}
            >
      ...
      <View style={styles.card}>
        <View style={styles.cardRow}>
          ...
          <View
            style={[
              styles.dot,
              { backgroundColor: STATUS_COLOR[syncStatus] || "#6B7280" },
            ]}
          />
          <Text
            style={[
              styles.statusText,
              { color: STATUS_COLOR[syncStatus] || "#6B7280" },
            ]}
          >
            {syncStatus}
          </Text>
        ...
        <Text style={styles.cardNote}>
          Port 7071 · Keeps all musician devices (phones / tablets) in sync
        </Text>
      ...
      {(() => {
        const sw =
          LYRIC_SOFTWARE_OPTIONS.find((o) => o.id === ppSoftware) ||
        ...
        return (
          <View style={styles.card}>
            ...
            <Text style={styles.cardNote}>
              The bridge translates CUE_CHANGE messages to your lyric software.
              Select the software in Settings.
            </Text>
            ...
            {LYRIC_SOFTWARE_OPTIONS.map((opt) => (
              <View
                ...
                style={[
                  styles.swRow,
                  opt.id === ppSoftware && styles.swRowActive,
                ]}
              >
                <Text
                  style={[
                    styles.swRowName,
                    opt.id === ppSoftware && styles.swRowNameActive,
                  ]}
                >
                  {opt.name}
                </Text>
            ...
            {ppTarget ? (
              ...
              <TouchableOpacity
                style={styles.ppSetupBtn}
                onPress={() => navigation.navigate("Settings")}
              >
                <Text style={styles.ppSetupText}>
                  Set target host / IP in Settings →
                </Text>
              </TouchableOpacity>
            )}
          </View>
      ...
      <View style={styles.hintCard}>
        ...
        <Text style={styles.hintText}>
          ...
          <Text style={styles.hintHighlight}>🎬 Lyric Cue Sync</Text>, and turn
          it ON. When the bridge is connected and you tap a section during live
          playback, the cue fires automatically to your lyric software.
        </Text>
      </View>
      ...
      <TouchableOpacity
        style={styles.bridgeSetupLink}
        onPress={() => navigation.navigate("BridgeSetup")}
      >
        <Text style={styles.bridgeSetupText}>
          Bridge setup guide & QR codes →
        </Text>
      </TouchableOpacity>
