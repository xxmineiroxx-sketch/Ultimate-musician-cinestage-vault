...
import {
  ...
  installPresetsFromBundle,
  presetInstallDir,
} from "../services/fatChannelPresets";
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.installCard}>
        ...
        <Text style={styles.subtitle}>
          Unzips the bundled library so presets are available offline for quick
        ...
        <View style={styles.installRow}>
          ...
          {installState.installed ? (
            <Text style={styles.installStatus}>
              Installed {installedCount} files
              {installedAt
        ...
        {installError ? (
          <Text style={styles.errorText}>
            Install error: {String(installError.message || installError)}
          </Text>
        ) : null}
      </View>
      {sections.map((section) => (
        <View key={section.category} style={styles.card}>
          <Text style={styles.cardTitle}>
            {section.category}{" "}
            <Text style={styles.cardMeta}>({section.presets.length})</Text>
          </Text>
          {section.presets.map((preset) => (
            <View
              key={`${section.category}-${preset.path}`}
              style={styles.itemRow}
            >
              <Text style={styles.itemText}>
                • {preset.subCategory ? `${preset.subCategory} — ` : ""}
                {preset.name}
              </Text>
              <Text style={styles.itemMeta}>
                {Math.max(1, Math.round(preset.sizeBytes / 1024))} KB
              </Text>
            </View>
          ))}
        </View>
