...
function parseSections(chordChart) {
  ...
  for (const line of lines) {
    ...
    const isHeader =
      /^(Intro|Verse|Chorus|Bridge|Outro|Pre-Chorus|Tag|Channel|Refr|Coro|Estrofe|Ponte|Alt Chorus)/i.test(
        trimmed,
      ) ||
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      <View style={s.infoRow}>
        {song?.key ? (
          <View style={s.badge}>
            <Text style={s.badgeText}>Key {song.key}</Text>
          </View>
        ) : null}
        {song?.bpm ? (
          <View style={s.badge}>
            <Text style={s.badgeText}>{song.bpm} BPM</Text>
          </View>
        ) : null}
        {song?.timeSig ? (
          <View style={s.badge}>
            <Text style={s.badgeText}>{song.timeSig}</Text>
          </View>
        ) : null}
        <View style={[s.badge, { borderColor: "#6366F1" }]}>
          <Text style={[s.badgeText, { color: "#A5B4FC" }]}>
      ...
      {sections.length === 0 ? (
        <View style={s.empty}>
          ...
          <Text style={s.emptyCaption}>
            Add a chord chart to this song to see the map.
          </Text>
        </View>
      ) : (
            ...
            {sections.map((sec, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  >
                    <Text style={[s.sectionLabel, { color: col.label }]}>
                      {sec.name}
                    </Text>
                    <Text style={s.sectionLineCount}>
                      {sec.lines.filter((l) => l.trim()).length} lines
                    </Text>
                  </View>
                  {i < sections.length - 1 && <Text style={s.arrow}>→</Text>}
                </React.Fragment>
              );
            })}
          </View>
            ...
            return (
              <View key={i} style={[s.detailCard, { borderColor: col.border }]}>
                <View style={[s.detailHeader, { backgroundColor: col.bg }]}>
                  <Text style={[s.detailTitle, { color: col.label }]}>
                    {sec.name}
                  </Text>
                </View>
                <Text style={s.detailContent} selectable>
                  {content}
                </Text>
              </View>
            );
