...
function normalizePlan(serviceId, plan) {
  ...
  return {
    ...
    updatedAt:
      Number.isFinite(base.updatedAt) && base.updatedAt > 0
        ? base.updatedAt
        : Date.now(),
  };
}
...
export async function addSongToService(serviceId, song) {
  ...
  const songId = String(song?.id || song?._sharedId || song?.songId || "").trim();
  if (!songId) {
    throw new Error("This song is missing an id and cannot be added.");
  }
  if (plan.songs.find((s) => s.songId === songId)) return plan; // already there
  const item = {
    id: uid("si"),
    songId,
...
export async function addVocalAssignment(
  serviceId,
  itemId,
  { personId, name, type, voicePart },
) {
  const plan = await getPlanForService(serviceId);
  return savePlanForService(serviceId, {
    ...plan,
...
export async function updateVoicePartAudio(
  serviceId,
  itemId,
  voicePart,
  audioInfo,
  ...
  return savePlanForService(serviceId, {
    ...
    songs: plan.songs.map((s) =>
      s.id === itemId
        ? {
            ...s,
            instrumentNotes: newInstrumentNotes,
            lyrics: lyricsOnly,
            distributedInKey: toKey,
          }
        : s,
    ),
  });
...
export async function removeVocalAssignment(serviceId, itemId, assignmentId) {
  ...
  return savePlanForService(serviceId, {
    ...
    songs: plan.songs.map((s) => {
      ...
      return {
        ...s,
        vocalAssignments: (s.vocalAssignments || []).filter(
          (v) => v.id !== assignmentId,
        ),
      };
    }),
