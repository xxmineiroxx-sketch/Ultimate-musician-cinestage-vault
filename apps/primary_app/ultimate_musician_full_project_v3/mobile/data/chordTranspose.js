...
function shouldUseFlatsForKey(key) {
...
const CHORD_TOKEN_PAT =
  /^[A-G][#b]?(m|M|maj|min|dim|aug|sus|add|dom)?[0-9]?(\/[A-G][#b]?)?$/;

// Matches chord tokens anywhere in a string (for substitution in chord lines)
const CHORD_IN_LINE_PAT =
  /[A-G][#b]?(m|M|maj|min|dim|aug|sus|add|dom)?[0-9]?(\/[A-G][#b]?)?/g;
...
export function transposeChordChart(chart, fromKey, toKey) {
  ...
  const flats = shouldUseFlatsForKey(toKey.trim());
  return chart
    .split("\n")
    .map((line) => {
      if (!isChordLine(line)) return line;
      return line.replace(CHORD_IN_LINE_PAT, (tok) =>
        transposeChordToken(tok, semitones, flats),
      );
    })
    .join("\n");
}
