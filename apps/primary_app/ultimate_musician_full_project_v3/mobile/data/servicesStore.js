...
export async function deleteService(serviceId) {
  ...
  await saveServices(next);
  const deleted = await getJSON(KEYS.DELETED_SERVICES, []);
  const nextDeleted = [...new Set([...(Array.isArray(deleted) ? deleted : []), serviceId].filter(Boolean))];
  await setJSON(KEYS.DELETED_SERVICES, nextDeleted);
  const active = await getActiveServiceId();
  if (active === serviceId) await setActiveServiceId("");
  return next;
}

export async function getDeletedServiceIds() {
  const deleted = await getJSON(KEYS.DELETED_SERVICES, []);
  return Array.isArray(deleted) ? deleted.filter(Boolean) : [];
}

export async function clearDeletedServiceIds() {
  await setJSON(KEYS.DELETED_SERVICES, []);
}
