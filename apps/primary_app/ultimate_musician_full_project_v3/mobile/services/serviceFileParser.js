...
export function parseServicePlanText(text) {
  ...
  try {
    ...
    if (Array.isArray(obj.planItems)) {
      return {
        items: obj.planItems.map((it, i) => ({
          title: it.title || it.name || it.itemTitle || `Item ${i + 1}`,
...
export function bestMatchSong(title, songs = []) {
  ...
  // exact
  const exact = songs.find((s) => (s.title || "").toLowerCase() === t);
  if (exact) return exact;
  // contains
  const contains = songs.find(
    (s) =>
  ...
  const norm = (x) =>
    (x || "")
      ...
      .trim();
  const nt = norm(title);
  const fuzzy = songs.find((s) => norm(s.title) === nt);
  return fuzzy || null;
}
