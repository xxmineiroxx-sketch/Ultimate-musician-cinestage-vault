...
export function speak(
  phrase,
  { rate = 0.95, pitch = 1.0, repeat = "ONCE" } = {},
) {
...
export function speakCountIn({
  ...
  const phrase =
    Array.from({ length: beats }, (_, i) => String(i + 1)).join(", ") +
    `, ${markerName}`;
...
export function scheduleCues({
  ...
  for (const m of safeMarkers) {
    ...
    if (m.countInBars && m.countInBars > 0) {
      const whenCountIn = Math.max(
        0,
        start - barsToSeconds(m.countInBars, bpm),
      );
      const tCount = setTimeout(() => {
        if (m.cueVisualOnly && onVisualCue)
          onVisualCue({
            marker: { ...m, name: `Count-in: ${m.name}` },
            whenSec: whenCountIn,
          });
        if (!m.cueVisualOnly) {
    ...
    const timer = setTimeout(() => {
      if (m.cueVisualOnly && onVisualCue)
        onVisualCue({ marker: m, whenSec: when });
      if (!m.cueVisualOnly && m.cueVoice)
        speak(formatCueText(m, cueTextMode), { repeat: m.cueRepeat || "ONCE" });
    }, when * 1000);
