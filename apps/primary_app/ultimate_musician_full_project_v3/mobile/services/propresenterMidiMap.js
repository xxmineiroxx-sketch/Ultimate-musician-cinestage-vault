...
export function buildProPresenterMidi({
  songIndex = 0,
  marker,
  midiConfig = {},
}) {
  ...
  const program = Number.isFinite(midiConfig.program)
    ? midiConfig.program
    : songIndex + 1;
  const ccSection = Number.isFinite(midiConfig.ccSection)
    ? midiConfig.ccSection
    : SECTION_CC;
  const ccSlide = Number.isFinite(midiConfig.ccSlide)
    ? midiConfig.ccSlide
    : SLIDE_CC;
  ...
  return {
    ...
    ccSlide:
      marker?.lyricsCue != null
        ? { cc: ccSlide, value: Number(marker.lyricsCue) }
        : null,
  };
}
