...
const STEM_ORDER = ["vocals", "drums", "bass", "guitars", "keys", "other"];
const STEM_LABELS = {
  vocals: "Vocals",
  drums: "Drums",
  bass: "Bass",
...
export function buildStemsFromArtifact(artifact, fallback = []) {
  const stems = artifact?.artifacts?.stems || artifact?.stems;
  if (!stems || typeof stems !== "object") return fallback;
  return STEM_ORDER.filter((key) => stems[key]).map((key) => ({
    id: `stem_${key}`,
