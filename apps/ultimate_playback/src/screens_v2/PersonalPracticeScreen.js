...
import { SYNC_URL, syncHeaders, CINESTAGE_URL } from '../../config/syncConfig';
import { calculateSemitoneShift } from '../utils/transpose';
import MasterpieceWaveform from '../components_v2/MasterpieceWaveform';
...
function resolveVocalBedUri(song, stems = {}, harmonies = {}, excludedUri = null) {
  // Only use truly instrumental sources — never the full mix or vocal stems,
  // because Track B is already the singer's vocal part. Playing both the full
  // song (which contains that vocal) AND the isolated vocal creates a comb
  // filter (= "robotic" effect). If no instrumental stems exist, return null
  // so Track A stays silent and the singer hears only their isolated part.
  const candidates = [
    getPlayableMediaUrl(stems.other),   // Demucs "other" = everything except vocals/drums/bass
...
function buildPersonalTracks(role, song) {
  ...
  //   - Never includes the full-song URL (would contain the player's part).
  //   - Never includes 'other' for instrument roles ('other' is a catch-all
  //     that may contain the player's instrument).
  //   - For vocal roles: all non-vocal stems (drums, bass, keys, guitars, other).
  //     'other' is safe here because it's the instrument bed, not vocals.
  //
  // Track B ("Your Part"):  ONLY the exact stem(s) for the assigned role.
  //   - No fallback to full song. If not available → show "Not available yet".
  // ──────────────────────────────────────────────────────────────────────────
  ...
  // ── Vocal roles (lead vocal, bgv, soprano, alto, tenor, etc.) ─────────────
  if (VOCAL_ROLES.has(role)) {
    // Track B: only the singer's specific vocal/harmony stem
    ...
    // Track A: all non-vocal stems (drums, bass, keys, guitars, other as bed)
    // 'other' is safe here — it's the instrumental bed that doesn't contain vocals.
    const instrumentalEntries = stemEntries.filter((entry) => !isVocalEntry(entry));

    // If no named instrument stems exist yet, try 'other' or specific instrument stems
    // as an instrument bed (from resolveVocalBedUri — no full-song fallback).
  ...
  // 'other' is excluded because it's a Demucs catch-all that may contain
  // the player's instrument — we can't safely include it.
  // For drums/bass (basic Demucs stems): 'other' still excluded so the player
  ...
  // Track B: ONLY the player's exact stem. No fallback — if not available, show empty.
  const trackA = makeTrackDef({
    id: 'full_mix',
    label: 'Full Song',
    sublabel: accompanimentEntries.length > 0
      ? `Everyone except ${stemLabel}`
      : 'Not available yet',
    entries: accompanimentEntries,
    externalUrl: null,
    color: '#3B82F6',
    icon: '🎸',
  });
...
export default function PersonalPracticeScreen({ route, navigation }) {
  ...
  // Pitch-shift stems when the service key differs from the original stem key.
  // Uses CineStage /stems/transpose (librosa phase-vocoder — no chipmunk, no tempo change).
  // Results are cached in R2 so second call for same song+semitones is instant.
  async function fetchTransposedStems(song, stems, harmonies) {
    const originalKey   = song?.key || '';           // library key = key stems were separated in
    const serviceKey    = song?.transposedKey || '';  // service-specific key override set by admin
    if (!serviceKey || !originalKey || serviceKey === originalKey) {
      return { stems, harmonies }; // no transposition needed
    }
    const semitones = calculateSemitoneShift(originalKey, serviceKey);
    if (!semitones || semitones === 0) return { stems, harmonies };

    // Only pitch-shift URLs (string values), skip null/object entries
    const toShift = {};
    ...
    try {
      ...
      if (!res.ok) return { stems, harmonies }; // fall back to originals on error
      const data = await res.json();
      const shifted = data?.stems || {};
      // Merge shifted URLs back — harmonies keep their original structure shape
      ...
      for (const [k, url] of Object.entries(shifted)) {
        ...
        if (k in harmoniesToShift) {
          newHarmonies[k] = typeof harmonies[k] === 'object'
            ? { ...harmonies[k], url }
            : url;
        }
      }
      ...
      console.warn('[PersonalPractice] transpose request failed:', e?.message);
      return { stems, harmonies }; // fall back gracefully
    }
  }
  ...
  async function selectSong(song, currentRole, options = {}) {
    ...
    if (lookupId) {
      ...
      if (Object.keys(stems).length > 0 || Object.keys(harmonies).length > 0) {
        // Pitch-shift stems if admin set a different service key
...
const styles = StyleSheet.create({
  ...
  mixerCard: {
    overflow: 'hidden',
    padding: 0, // Let the inner components handle padding
  },
  mixerTitle: {
    color: '#64748B', fontSize: 11, fontWeight: '700',
