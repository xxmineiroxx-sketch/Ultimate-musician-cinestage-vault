...
const withSongRevision = (song = {}, previous = null) => {
  ...
  return {
    ...merged,
    contentHash,
    revision: Math.max(0, Number(previous?.revision || previous?.songRevision || song.revision || 0) || 0) + (changed ? 1 : 0),
    songRevision: Math.max(0, Number(previous?.revision || previous?.songRevision || song.revision || 0) || 0) + (changed ? 1 : 0),
    updated_at: stamp,
    updatedAt: stamp,
    lastEditedAt: stamp,
